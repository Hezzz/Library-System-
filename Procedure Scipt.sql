...
create or replace PROCEDURE check_reserve
    IS
    patID       library_transaction.patron_id%TYPE;
    isbn_num    library_transaction.isbn_number%TYPE;
    shlf_id     library_transaction.shelf_id%TYPE;
    cpy_no      library_transaction.book_copy_number%TYPE;
BEGIN
    SELECT patron_id, isbn_number, shelf_id, book_copy_number
    INTO patID, isbn_num, shlf_id, cpy_no
    FROM library_transaction
    WHERE SYSDATE > book_return_date
    AND book_return_status = 'on-loan';
    
    UPDATE library_transaction
    SET book_return_status = 'returned'
    WHERE SYSDATE > book_return_date
    AND book_return_status = 'on-loan';
    ...
    UPDATE book
        ...
        AND shelf_id = shlf_id
        AND copy_number = cpy_no;
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Reservation checked.');
END;
...
create or replace PROCEDURE reserve_book
    ...
    IS
        ...
        book_status book.current_status%TYPE;
        bkTitle book.book_title%TYPE;
        auth_id book.author_author_id%TYPE;
        e_book_on_loan EXCEPTION;
        e_max_books EXCEPTION;
BEGIN
    SELECT current_status INTO book_status FROM book
    ...
    ELSE
        ...
        IF pat_books <> 2 THEN
            ...
            SELECT author_author_id, book_title INTO auth_id, bkTitle
                FROM book 
                WHERE book_isbn_number = isbnNum 
                AND shelf_id = shf_ID 
                AND copy_number = cpyNo
            ...
            INSERT INTO library_transaction VALUES(transaction_id.nextval, patID, auth_id,
                    isbnNum, bkTitle, shf_ID, cpyNo, 'admin', 'on-hold', SYSDATE, SYSDATE+7);
...
create or replace PROCEDURE return_book
    ( patID     IN library_transaction.patron_id%TYPE,
...
create or replace PROCEDURE withdraw_book(patID IN patrons.loginid%TYPE, 
    ...
    IS
        ...
        pat_books   patrons.borrowed_books_count%TYPE;
        book_status book.current_status%TYPE;
        trans_ID    library_transaction.transaction_id%TYPE;
        e_no_reserved EXCEPTION;
        isAvailable NUMBER;
BEGIN
    SELECT COUNT(*) INTO isAvailable FROM library_transaction
        WHERE patron_id = patID
        AND book_title = bkTitle
        AND shelf_id = shlfID
        AND book_copy_number = cpyNo
        AND book_return_status = 'on-hold';
        
    SELECT transaction_id INTO trans_ID FROM library_transaction
        WHERE patron_id = patID
        AND book_title = bkTitle
        AND shelf_id = shlfID
        AND book_copy_number = cpyNo
    ...
    IF isAvailable = 1 THEN
        UPDATE library_transaction
        SET book_return_status = 'on-loan',
            book_borrow_date = SYSDATE,
            book_return_date = SYSDATE + 7,
            librarian_id = libID
        WHERE patron_id = patID
        AND book_title = bkTitle
        AND shelf_id = shlfID
